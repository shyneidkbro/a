...
local genesislib = {
  Themes = {
   ...
   Iron = {
    ["Color Hub 1"] = ColorSequence.new({
      ColorSequenceKeypoint.new(0.00, Color3.fromRGB(32, 33, 36)),   -- ferro bruto / canto inferior-esquerdo
      ColorSequenceKeypoint.new(0.50, Color3.fromRGB(110, 112, 118)), -- zinco médio
      ColorSequenceKeypoint.new(1.00, Color3.fromRGB(228, 230, 235))  -- brilho reluzente / canto superior-direito
    }),
    ["Color Hub 2"] = Color3.fromRGB(38, 39, 43),    -- base cinza-escuro metálico
    ["Color Stroke"] = Color3.fromRGB(82, 85, 92),   -- contorno cinza-aço
    ["Color Theme"] = Color3.fromRGB(176, 186, 196), -- destaque prateado/azul-zinco (parece reflexo de metal polido)
    ["Color Text"] = Color3.fromRGB(248, 249, 250),  -- quase branco puro
    ["Color Dark Text"] = Color3.fromRGB(160, 163, 170) -- cinza claro para descrições
  },
  PurpleFlex = {
    ["Color Hub 1"] = ColorSequence.new({
      ColorSequenceKeypoint.new(0.00, Color3.fromRGB(18, 8, 28)),    -- preto profundo com sutil tom roxo
      ColorSequenceKeypoint.new(0.50, Color3.fromRGB(45, 18, 75)),   -- roxo escuro metálico
      ColorSequenceKeypoint.new(1.00, Color3.fromRGB(95, 35, 150))   -- reflexo roxo vibrante
    }),
    ["Color Hub 2"] = Color3.fromRGB(26, 12, 40),     -- base roxo muito escuro
    ["Color Stroke"] = Color3.fromRGB(65, 28, 100),   -- contorno roxo profundo
    ["Color Theme"] = Color3.fromRGB(147, 51, 255),   -- destaque roxo neon/flex
    ["Color Text"] = Color3.fromRGB(245, 240, 255),   -- branco com leve tom lilás
    ["Color Dark Text"] = Color3.fromRGB(155, 130, 195) -- lilás acinzentado para descrições
  }
},
  Info = {
    Version = "1.1.0"
...
local Settings = genesislib.Settings
local Flags = genesislib.Flags

local SetProps, SetChildren, InsertTheme, Create do
  InsertTheme = function(Instance, Type)
    table.insert(genesislib.Instances, {
      Instance = Instance,
      Type = Type
    })
    return Instance
  ...
  local function Save(file)
    if readfile and isfile and isfile(file) then
      ...
      if type(decode) == "table" then
        if rawget(decode, "UISize") then genesislib.Save["UISize"] = decode["UISize"] end
        if rawget(decode, "TabSize") then genesislib.Save["TabSize"] = decode["TabSize"] end
        if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then genesislib.Save["Theme"] = decode["Theme"] end
      end
    end
  end

  pcall(Save, "genesis_lib.json")
end
...
local Connections, Connection = {}, genesislib.Connection do
...
local function GetUIParent()
  ...
  if syn and syn.protect_gui then
    -- protect after create
    return CoreGui
  end
  return CoreGui
end

local ScreenGui = Create("ScreenGui", GetUIParent(), {
  Name = "GenesisLib",
  ResetOnSpawn = false,
  ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
  DisplayOrder = 999999999, -- acima de CoreGui / outras UIs
  IgnoreGuiInset = true
}, {
  Create("UIScale", {
    Scale = UIScale,
...
local function DestroyOld()
  ...
  for _, child in ipairs(parent:GetChildren()) do
    if child.Name == ScreenGui.Name and child ~= ScreenGui and child:IsA("ScreenGui") then
      child:Destroy()
    end
  end
end
...
local function VerifyTheme(Theme)
  for name,_ in pairs(genesislib.Themes) do
    if name == Theme then
      return true
    end
  end
...
local Theme = genesislib.Themes[genesislib.Save.Theme]

local function AddEle(Name, Func)
  genesislib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
  local Element = genesislib.Elements[Ele](Instance, props, ...)
  return Element
end
...
-- Cache de URLs já resolvidas (evita re-download)
genesislib._ImageCache = genesislib._ImageCache or {}
genesislib.AssetCacheFolder = "GenesisLib/Assets"

local function GenesisHttpGetBytes(url)
  local methods = {
    function() return game:HttpGet(url) end,
    function() return game:HttpGetAsync(url) end,
  }
  if request then
    table.insert(methods, 1, function()
...
-- Resolve qualquer imagem: rbxassetid, número, URL http(s), nome de ícone lucide
-- Retorna string usável em ImageLabel.Image / ImageButton.Image
function genesislib:ResolveImage(img, Configs)
  Configs = Configs or {}
  if type(img) ~= "string" or img:len() < 1 then return "" end

  if img:find("rbxassetid://") or img:find("rbxasset://") or img:find("rbxthumb://") then
    return img
  end
  ...
  -- URL http(s) → download + getcustomasset
  if img:find("^https?://") then
    if genesislib._ImageCache[img] then
      return genesislib._ImageCache[img]
    end

    local AutoDownload = Configs.AutoDownload ~= false
    local CacheFolder = Configs.CacheFolder or genesislib.AssetCacheFolder or "GenesisLib/Assets"

    if not AutoDownload then
      return img
    end

    local fileName = img:match(".+/(.-)$") or "image.png"
    ...
    if isfile and isfile(fullPath) and getcustomasset then
      local ok, asset = pcall(getcustomasset, fullPath)
      if ok and asset then
        genesislib._ImageCache[img] = asset
        return asset
      end
    end

    local content = GenesisHttpGetBytes(img)
    if content then
      if makefolder then pcall(makefolder, CacheFolder) end
      if writefile then
        pcall(writefile, fullPath, content)
    ...
    genesislib._ImageCache[img] = img
    return img
  end

  -- nome de ícone da lib (lucide etc.)
  local lower = img:lower():gsub("lucide", ""):gsub("-", "")
  for Name, Icon in pairs(genesislib.Icons) do
...
function genesislib:GetIcon(IconName)
  if type(IconName) ~= "string" or IconName:len() < 1 then return IconName or "" end
  -- passa por ResolveImage (URL, assetid, número, nome)
  return genesislib:ResolveImage(IconName)
end

function genesislib:SetTheme(NewTheme)
  if not VerifyTheme(NewTheme) then return end

  genesislib.Save.Theme = NewTheme
  SaveJson("GenesisLib.json", genesislib.Save)
  Theme = genesislib.Themes[NewTheme]

  Connection:FireConnection("ThemeChanged", NewTheme)
  table.foreach(genesislib.Instances, function(_,Val)
...
function genesislib:SetScale(NewScale)
  NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
  UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function genesislib:MakeWindow(Configs)
  local WTitle = Configs[1] or Configs.Name or Configs.Title or "Genesis Lib"
  local WMiniText = Configs[2] or Configs.SubTitle or "by : sh2.cc"

  Settings.ScriptFile = Configs[3] or Configs.SaveFolder or false

  -- Glass / Blur (0 = off, 0.1~1 = intensidade)
  local WGlass = tonumber(Configs.GlassEffect or Configs.Glass or 0) or 0
  local WBlur = tonumber(Configs.Blur or Configs.BackgroundBlur or 0) or 0
  WGlass = math.clamp(WGlass, 0, 1)
  WBlur = math.clamp(WBlur, 0, 1)
  ...
  local UISizeX, UISizeY = unpack(genesislib.Save.UISize)
  local MainFrame = InsertTheme(Create("ImageButton", ScreenGui, {
    Size = UDim2.fromOffset(UISizeX, UISizeY),
    Position = UDim2.new(0.5, -UISizeX/2, 0.5, -UISizeY/2),
    BackgroundTransparency = 1,
    Name = "Hub",
    ZIndex = 1 -- abaixo das Notifications (ZIndex 1000)
  }), "Main")
  MakeDrag(MainFrame)

  local MainCorner = Make("Corner", MainFrame)

  -- Window table precisa existir ANTES de atribuir BackgroundImage/ThemeBg/Overlay
  local Window = {}
  local FirstTab = false

  -- Camada de fundo (wallpaper + tint). ZIndex baixo.
  -- Overlay fica DENTRO desta camada → nunca cobre os elementos da UI.
  local BackgroundLayer = Create("Frame", MainFrame, {
  ...
  Make("Corner", BackgroundLayer, UDim.new(0, 7))

  -- Wallpaper
  Window.BackgroundImage = Create("ImageLabel", BackgroundLayer, {
  ...
  Make("Corner", Window.BackgroundImage, UDim.new(0, 7))

  -- Fundo sólido/gradiente do tema (visível quando NÃO tem wallpaper)
  Window.ThemeBg = InsertTheme(Create("Frame", BackgroundLayer, {
    Name = "ThemeBg",
    Size = UDim2.new(1, 0, 1, 0),
    BackgroundTransparency = 0.03,
    ZIndex = 2
  }), "Main")
  Make("Gradient", Window.ThemeBg, {Rotation = 45})
  Make("Corner", Window.ThemeBg, UDim.new(0, 7))

  -- Tint escuro SÓ sobre o wallpaper (irmão do ThemeBg, abaixo dos Components)
  Window.BackgroundOverlay = InsertTheme(Create("Frame", BackgroundLayer, {
    Name = "BackgroundOverlay",
    Size = UDim2.new(1, 0, 1, 0),
    BackgroundColor3 = Theme["Color Hub 2"],
    BackgroundTransparency = 1, -- transparente até SetBackground
    ZIndex = 3
  }), "Frame")
  Make("Corner", Window.BackgroundOverlay, UDim.new(0, 7))

  -- Camada "Glass" (vidro fosco sobre o wallpaper, abaixo da UI)
  Window.GlassLayer = Create("Frame", BackgroundLayer, {
    Name = "GlassLayer",
    Size = UDim2.new(1, 0, 1, 0),
    BackgroundColor3 = Color3.fromRGB(255, 255, 255),
    BackgroundTransparency = 1, -- off por padrão
    BorderSizePixel = 0,
    ZIndex = 4
  })
  Make("Corner", Window.GlassLayer, UDim.new(0, 7))
  -- stroke sutil estilo glass
  local GlassStroke = Create("UIStroke", Window.GlassLayer, {
    Name = "GlassStroke",
  ...
  -- UI Components SEMPRE acima da BackgroundLayer
  local Components = Create("Frame", MainFrame, {
    Name = "Components",
    Size = UDim2.new(1, 0, 1, 0),
    BackgroundTransparency = 1,
    ZIndex = 10
  ...
  -- estado interno de efeitos
  Window._glassLevel = WGlass
  Window._blurLevel = WBlur
  Window._bgOverlayTransparency = 0.45 -- default mais escuro que 0.35
  Window._bgImageTransparency = 0
  Window._lightingBlur = nil
  ...
  local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
    Size = UDim2.new(0, genesislib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
  ...
  ConnectSave(ControlSize1, function()
    if not Minimized then
      genesislib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
      SaveJson("Genesis library Beta.json", genesislib.Save)
    end
  end)

  ConnectSave(ControlSize2, function()
    genesislib.Save.TabSize = MainScroll.Size.X.Offset
    SaveJson("Genesis library Beta.json", genesislib.Save)
  end)
  ...
  -- Window / FirstTab já declarados no topo do MakeWindow
  ...
  function Window:MinimizeBtn()
    ...
    -- se tem diálogo aberto, fecha ao minimizar
    local dlg = MainFrame:FindFirstChild("Dialog") or ScreenGui:FindFirstChild("Dialog")
    if dlg then
      if Window._activeDialog and Window._activeDialog.Close then
        pcall(function() Window._activeDialog:Close() end)
      else
    ...
    if Minimized then
      ...
      -- Restaurar layout normal com animação suave
      local iconX = TitleIcon and TitleIcon.Position.X.Offset or 0
      local titleX = TitleIcon and (iconX + 35) or 15
      CreateTween({Title, "Position", UDim2.new(0, titleX, 0.6, 0), 0.25})
      if TitleIcon then
        CreateTween({TitleIcon, "Position", UDim2.new(0, iconX, 0.6, 0), 0.25})
      end
      -- SubTitle reaparece com fade in
      Title.SubTitle.Visible = true
      CreateTween({Title.SubTitle, "TextTransparency", 0, 0.2})
    else
      ...
      -- Centralizar tudo no header (28px) com animação
      local iconX = TitleIcon and TitleIcon.Position.X.Offset or 0
      local titleX = TitleIcon and (iconX + 35) or 15
      CreateTween({Title, "Position", UDim2.new(0, titleX, 0.5, 0), 0.25})
      if TitleIcon then
        CreateTween({TitleIcon, "Position", UDim2.new(0, iconX, 0.5, 0), 0.25})
      end
      -- SubTitle some com fade out
      CreateTween({Title.SubTitle, "TextTransparency", 1, 0.2})
      task.delay(0.2, function()
        if Minimized then Title.SubTitle.Visible = false end
      end)
    end
  ...
  function Window:AddMinimizeButton(Configs)
    ...
    -- resolve URL/asset no ícone do botão
    if Button:IsA("ImageButton") or Button:IsA("ImageLabel") then
      local img = (Configs.Button and Configs.Button.Image) or Button.Image
      if img and tostring(img):len() > 0 then
        Button.Image = genesislib:ResolveImage(tostring(img))
      end
    end
  ...
  function Window:AddIconBehindOfTitle(Configs)
    local IconConfig = Configs.Icon or {}
    local CornerConfig = Configs.Corner
    local TitleOffset = Configs.TitleOffset or 35

    local IconImage = genesislib:ResolveImage(IconConfig.Image or "")
    local IconSize = IconConfig.Size or UDim2.new(0, 22, 0, 22)
    local IconPos = IconConfig.Position or UDim2.new(0, 10, 0.6)

    TitleIcon = Create("ImageLabel", TopBar, {
      Name = "TitleIcon",
      Size = IconSize,
      Position = IconPos,
      AnchorPoint = Vector2.new(0, 0.5),
      Image = IconImage,
      BackgroundTransparency = 1,
      ImageColor3 = IconConfig.ImageColor3 or Color3.fromRGB(255, 255, 255)
    })

    if CornerConfig then
      local Corner = Make("Corner", TitleIcon)
      SetProps(Corner, CornerConfig)
    end

    -- Empurra o título para a direita para não sobrepor o ícone
    Title.Position = UDim2.new(0, IconPos.X.Offset + TitleOffset, 0.6)

    local IconObj = {}
    function IconObj:SetImage(NewImage)
      TitleIcon.Image = genesislib:ResolveImage(NewImage or "")
    end
    function IconObj:SetSize(NewSize)
      TitleIcon.Size = NewSize
    end
    function IconObj:SetPosition(NewPos)
      TitleIcon.Position = NewPos
      Title.Position = UDim2.new(0, NewPos.X.Offset + TitleOffset, 0.6)
    end
  ...
  function Window:Dialog(Configs)
    -- fecha diálogo anterior se existir (hub ou tela cheia)
    local existing = MainFrame:FindFirstChild("Dialog") or ScreenGui:FindFirstChild("Dialog")
    if existing then
    ...
    -- Backdrop em TELA CHEIA (ScreenGui) → escurece CoreGui/toda a tela, não só o hub
    local Screen = Create("TextButton", ScreenGui, {
      Name = "Dialog",
      Size = UDim2.new(1, 0, 1, 0),
      Position = UDim2.new(0, 0, 0, 0),
      BackgroundColor3 = Color3.fromRGB(0, 0, 0),
      BackgroundTransparency = 1, -- anima pra ~0.5
      Text = "",
      AutoButtonColor = false,
      Active = true,
      ZIndex = 2500
    ...
    CreateTween({Screen, "BackgroundTransparency", 0.4, 0.18})
    CreateTween({Frame, "Size", UDim2.fromOffset(250, 150), 0.2})

    local ButtonCount, Dialog, closed = 1, {}, false
    function Dialog:Close()
      if closed then return end
      closed = true
      pcall(function()
        CreateTween({Frame, "Size", UDim2.fromOffset(250 * 1.08, 150 * 1.08), 0.15})
    ...
    -- clicar no backdrop (fora do card) fecha
    Screen.MouseButton1Click:Connect(function()
      Dialog:Close()
    end)
    -- impede que clique no card feche
    ...
    Window._activeDialog = Dialog
    return Dialog
  end
  function Window:SelectTab(TabSelect)
    if type(TabSelect) == "number" then
      genesislib.Tabs[TabSelect].func:Enable()
    else
      for _,Tab in pairs(genesislib.Tabs) do
  ...
  function Window:MakeTab(paste, Configs)
    ...
    TIcon = genesislib:GetIcon(TIcon)
    ...
    local function Tabs()
      ...
      table.foreach(genesislib.Tabs, function(_,Tab)
    ...
    FirstTab = true
    local Tab = {}
    table.insert(genesislib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
    Tab.Cont = Container
    ...
    function Tab:AddSection(Configs)
      ...
      local Section = {}
      table.insert(genesislib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    function Tab:AddDropdown(Configs)
      ...
      -- ===== Modal Dropdown (centralizado + backdrop, estilo diálogo) =====
      ...
      local DropTitle = InsertTheme(Create("TextLabel", DropHeader, {
      ...
      local DropClose = Create("ImageButton", DropHeader, {
      ...
      local function CloseDropdown()
        ...
        CreateTween({NoClickFrame, "BackgroundTransparency", 1, 0.15})
        CreateTween({DropFrame, "Size", UDim2.fromOffset(280 * 1.06, 320 * 1.06), 0.15})
        CreateTween({DropFrame, "BackgroundTransparency", 1, 0.15})
        task.delay(0.16, function()
          NoClickFrame.Visible = false
          if DropFrame then
            DropFrame.BackgroundTransparency = 0
            DropFrame.Size = UDim2.fromOffset(280, 320)
          end
          WaitClick = false
        end)
      end
      ...
      local function OpenDropdown()
        if IsOpen or WaitClick then return end
        IsOpen = true
        WaitClick = true
        DropTitle.Text = DName
        NoClickFrame.Visible = true
        NoClickFrame.BackgroundTransparency = 1
        DropFrame.Size = UDim2.fromOffset(280 * 1.06, 320 * 1.06)
        DropFrame.BackgroundTransparency = 0.2
        Arrow.Image = "rbxassetid://10709790948"
        CreateTween({Arrow, "ImageColor3", Theme["Color Theme"], 0.15})
        CreateTween({NoClickFrame, "BackgroundTransparency", 0.4, 0.18})
        CreateTween({DropFrame, "Size", UDim2.fromOffset(280, 320), 0.18})
        CreateTween({DropFrame, "BackgroundTransparency", 0, 0.15})
        task.delay(0.05, function() WaitClick = false end)
      end
      ...
      local function CalculateSize() end -- modal tem tamanho fixo
      local function CalculatePos() end -- centralizado

      -- fechar: clique no backdrop ou no X
      NoClickFrame.MouseButton1Click:Connect(function()
        CloseDropdown()
      end)
      DropClose.Activated:Connect(function()
        CloseDropdown()
      end)
      -- clique no card não fecha
      DropFrame.InputBegan:Connect(function() end)
      ...
      local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected do
        ...
        -- Normaliza Default/Flag pra MultiSelect (array JSON ou mapa {Name=true})
        local function IsInOptions(name)
          if DOptions[name] ~= nil then return true end
          if table.find(DOptions, name) then return true end
          for _, opt in pairs(DOptions) do
            if tostring(opt) == tostring(name) then return true end
          end
          return false
        end

        if MultiSelect then
          local source = (CheckFlag(Flag) and GetFlag(Flag)) or Default
          ...
          if type(source) == "table" then
            for index, Value in pairs(source) do
              if type(index) == "string" then
                -- mapa: {Fight = true, Power = false}
                if Value and IsInOptions(index) then
                  Selected[index] = true
                end
              elseif type(Value) == "string" and IsInOptions(Value) then
                -- array: {"Fight", "Power"}
                Selected[Value] = true
              elseif type(Value) == "table" then
                -- nested raro
                for _, v2 in pairs(Value) do
                  if type(v2) == "string" and IsInOptions(v2) then
                    Selected[v2] = true
                  end
                end
              end
      ...
      function Dropdown:Set(Val1, Clear)
        if type(Val1) == "function" then
          Callback = Val1
          return
        end
        if type(Val1) == "table" then
          -- se parecer lista de opções novas (valores que não estão selected-like)
          -- suporte: Set({"Fight","Power"}) em MultiSelect = seleciona esses
          if MultiSelect then
            local looksLikeSelection = true
            for k, v in pairs(Val1) do
              if type(k) == "number" and type(v) == "string" then
                -- array de nomes selecionados
              elseif type(k) == "string" and type(v) == "boolean" then
                -- mapa
              else
                looksLikeSelection = false
                break
              end
            end
            if looksLikeSelection and not Clear then
              -- limpa seleção atual
              ...
              for k, v in pairs(Val1) do
                ...
                local on = type(k) == "string" and (v == true) or true
                if type(name) == "string" and Options[name] then
                  Options[name].Stats = on and true or false
        ...
        elseif type(Val1) == "string" and not MultiSelect then
          for _, Val in pairs(Options) do
            if Val.Name == Val1 or Val.Value == Val1 then
              Select(Val)
              break
            end
          end
      ...
      -- retorna seleção atual (Multi: array de strings; Single: string)
    ...
    function Tab:AddDiscordInvite(Configs)
      ...
      local Logo = genesislib:ResolveImage(Configs[2] or Configs.Logo or "")
      local Invite = Configs[3] or Configs.Invite or ""
      local Banner = Configs.Banner and genesislib:ResolveImage(Configs.Banner) or nil
      local JoinText = Configs.Join or Configs.JoinText or "Join"
      local HideInvite = Configs.HideInvite == true
      local hasBanner = Banner and type(Banner) == "string" and #Banner > 0

      -- cor do botão (default = verde escuro original da lib)
      local BtnColor = Configs.ButtonColor or Configs.JoinColor or Color3.fromRGB(50, 150, 50)
      local BtnTextColor = Configs.ButtonTextColor or Configs.JoinTextColor or Color3.fromRGB(220, 220, 220)
      local JoinIcon = Configs.JoinIcon or Configs.ButtonIcon or Configs.Icon
      if JoinIcon then
        JoinIcon = genesislib:ResolveImage(tostring(JoinIcon))
      end
      local hasJoinIcon = JoinIcon and type(JoinIcon) == "string" and #JoinIcon > 0

      local linkH = HideInvite and 0 or 15
      local bannerH = hasBanner and 48 or 0
      local bodyH = 58
      local cardH = bannerH + bodyH
      local totalH = linkH + cardH + (HideInvite and 0 or 2)
      ...
      local FrameHolder = InsertTheme(Create("Frame", InviteHolder, {
        ...
        Position = UDim2.new(0, 0, 0, linkH),
        BackgroundColor3 = Theme["Color Hub 2"],
        ClipsDescendants = true
      }), "Frame")
      ...
      if hasBanner then
        local BannerImg = Create("ImageLabel", FrameHolder, {
          Name = "Banner",
          Size = UDim2.new(1, 0, 0, bannerH + 10),
          Position = UDim2.new(0, 0, 0, 0),
          Image = Banner,
          BackgroundColor3 = Color3.fromRGB(30, 30, 35),
          BackgroundTransparency = 0,
          ScaleType = Enum.ScaleType.Crop,
          ImageColor3 = Color3.fromRGB(160, 160, 170), -- um pouco mais escuro
        ...
        local BannerDim = Create("Frame", FrameHolder, {
          Name = "BannerDim",
          Size = UDim2.new(1, 0, 0, bannerH + 10),
          Position = UDim2.new(0, 0, 0, 0),
          BackgroundColor3 = Color3.fromRGB(0, 0, 0),
        ...
        Make("Corner", BannerDim, UDim.new(0, 8))

        local Fade = Create("Frame", FrameHolder, {
          Name = "BannerFade",
          Size = UDim2.new(1, 0, 0, 18),
          Position = UDim2.new(0, 0, 0, bannerH - 8),
          BackgroundColor3 = Theme["Color Hub 2"],
          BorderSizePixel = 0,
          ZIndex = 3
        })
      ...
      local ContentY = hasBanner and (bannerH - 14) or 8
      local LogoSize = hasBanner and 36 or 30
      ...
      local ImageLabel = Create("ImageLabel", FrameHolder, {
        ...
        Image = Logo or "",
        BackgroundColor3 = Theme["Color Hub 2"],
        BackgroundTransparency = (Logo and Logo ~= "") and 1 or 0,
        ZIndex = 5
      })
      Make("Corner", ImageLabel, UDim.new(0, hasBanner and 10 or 6))
      local logoStroke = Make("Stroke", ImageLabel)
      if logoStroke then
        logoStroke.Thickness = hasBanner and 2 or 1
        logoStroke.Color = Theme["Color Hub 2"]
      end

      local textX = 10 + LogoSize + 10
      InsertTheme(Create("TextLabel", FrameHolder, {
        Size = UDim2.new(1, -(textX + 8), 0, 14),
        Position = UDim2.new(0, textX, 0, ContentY + (hasBanner and 4 or 0)),
        Font = Enum.Font.GothamBold,
        TextColor3 = Theme["Color Text"],
        TextXAlignment = "Left",
        BackgroundTransparency = 1,
      ...
      InsertTheme(Create("TextLabel", FrameHolder, {
        Size = UDim2.new(1, -(textX + 8), 0, 0),
        Position = UDim2.new(0, textX, 0, ContentY + (hasBanner and 18 or 15)),
        TextWrapped = true,
        AutomaticSize = "Y",
        Font = Enum.Font.Gotham,
        TextColor3 = Theme["Color Dark Text"],
      ...
      local JoinButton = Create("TextButton", FrameHolder, {
        ...
        Text = hasJoinIcon and "" or JoinText,
        Font = Enum.Font.GothamBold,
        TextSize = 11,
      ...
      local JoinIconLabel = nil
      if hasJoinIcon then
        -- botão com ícone + texto
        JoinButton.Text = ""
        local row = Create("Frame", JoinButton, {
          Size = UDim2.new(1, 0, 1, 0),
          BackgroundTransparency = 1,
          ZIndex = 7
        })
        ...
        JoinIconLabel = Create("ImageLabel", row, {
          Size = UDim2.fromOffset(14, 14),
          Image = JoinIcon,
          BackgroundTransparency = 1,
          ImageColor3 = BtnTextColor,
          ZIndex = 8
        })
      ...
      local function SetBtnVisual(color, textColor, label)
        JoinButton.BackgroundColor3 = color
        JoinButton.TextColor3 = textColor
        if hasJoinIcon then
          JoinButton.Text = ""
          local lbl = JoinButton:FindFirstChild("JoinLabel", true)
          if lbl then lbl.Text = label end
          if JoinIconLabel then JoinIconLabel.ImageColor3 = textColor end
          if lbl then lbl.TextColor3 = textColor end
        else
          JoinButton.Text = label
        end
      end

      local hoverColor = Color3.new(
        math.min(1, BtnColor.R * 1.15 + 0.05),
        math.min(1, BtnColor.G * 1.15 + 0.05),
        math.min(1, BtnColor.B * 1.15 + 0.05)
      )
      ...
      JoinButton.MouseEnter:Connect(function()
        ...
        pcall(function()
          CreateTween({JoinButton, "BackgroundColor3", hoverColor, 0.15})
        end)
      end)
      JoinButton.MouseLeave:Connect(function()
        if ClickDelay then return end
        pcall(function()
          CreateTween({JoinButton, "BackgroundColor3", BtnColor, 0.15})
        end)
      end)

      JoinButton.Activated:Connect(function()
        if Invite and #tostring(Invite) > 0 then
          pcall(function() setclipboard(tostring(Invite)) end)
        end
        if ClickDelay then return end
        ...
        task.wait(2.5)
        SetBtnVisual(BtnColor, BtnTextColor, JoinText)
        ClickDelay = false
      end)
      ...
      function DiscordInvite:SetJoinText(t)
        JoinText = tostring(t or "Join")
        if not ClickDelay then SetBtnVisual(BtnColor, BtnTextColor, JoinText) end
      end
      function DiscordInvite:SetButtonColor(c, tc)
        if c then BtnColor = c end
        if tc then BtnTextColor = tc end
        if not ClickDelay then SetBtnVisual(BtnColor, BtnTextColor, JoinText) end
      end
      function DiscordInvite:SetBanner(img)
        local resolved = img and genesislib:ResolveImage(img) or ""
        local b = FrameHolder:FindFirstChild("Banner")
        if b and b:IsA("ImageLabel") then
          b.Image = resolved or ""
        end
      end
      function DiscordInvite:SetLogo(img)
        ImageLabel.Image = genesislib:ResolveImage(img) or ""
      end
      function DiscordInvite:SetJoinIcon(img)
        local resolved = img and genesislib:ResolveImage(img) or ""
        if JoinIconLabel then
          JoinIconLabel.Image = resolved or ""
        end
      end
      return DiscordInvite
    end

    function Tab:AddEventCard(Configs)
      local CTitle = Configs[1] or Configs.Title or "Event"
      local CSubtitles = Configs[2] or Configs.Subtitles or {}
      local CImage = Configs[3] or Configs.Image or ""
      local CButtonText = Configs[4] or Configs.ButtonText or "Join"
      local CButtonIcon = Configs[5] or Configs.ButtonIcon or Configs.Icon or nil
      local Callback = Configs[6] or Configs.Callback or function() end

      -- Resolve images
      if not CImage:find("rbxassetid://") then
        CImage = genesislib:GetIcon(CImage)
      end
      if CButtonIcon and not CButtonIcon:find("rbxassetid://") then
        CButtonIcon = genesislib:GetIcon(CButtonIcon)
      end

      local CardHolder = Create("Frame", Container, {
      ...
      local CardFrame = InsertTheme(Create("Frame", CardHolder, {
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = "Y",
        BackgroundColor3 = Theme["Color Hub 2"],
        Name = "EventCard"
      }), "Frame")
      Make("Corner", CardFrame, UDim.new(0, 6))

      Create("UIPadding", CardFrame, {
      ...
      Create("UIListLayout", CardFrame, {
        SortOrder = "LayoutOrder",
        Padding = UDim.new(0, 5)
      })

      -- Top row: Image + Text + Button (mais fino: 32px)
      local TopRow = Create("Frame", CardFrame, {
        Size = UDim2.new(1, 0, 0, 32),
        BackgroundTransparency = 1,
        LayoutOrder = 1
      })

      local CardImage = Create("ImageLabel", TopRow, {
      ...
      Make("Corner", CardImage, UDim.new(0, 5))

      -- Calcular largura do botão dinamicamente
      local hasBtnText = CButtonText and CButtonText:gsub(" ", ""):len() > 0
      local hasBtnIcon = CButtonIcon and CButtonIcon:find("rbxassetid://")
      local btnWidth = 0
      if hasBtnText and hasBtnIcon then
        btnWidth = 58
      elseif hasBtnText then
        btnWidth = 48
      elseif hasBtnIcon then
        btnWidth = 26
      else
        btnWidth = 48
      end

      local TextHolder = Create("Frame", TopRow, {
        Size = UDim2.new(1, -(btnWidth + 42), 1, 0),
        Position = UDim2.new(0, 36, 0, 0),
        BackgroundTransparency = 1,
        Name = "TextHolder"
      })

      Create("UIListLayout", TextHolder, {
        SortOrder = "LayoutOrder",
        VerticalAlignment = "Center",
      ...
      -- Subtitles (guardar referências pra edição em tempo real)
      local SubtitleLabels = {}
      for i, sub in ipairs(CSubtitles) do
        local lbl = InsertTheme(Create("TextLabel", TextHolder, {
        ...
        table.insert(SubtitleLabels, lbl)
      end

      -- ActionButton com suporte a ícone opcional
      local ActionButton = InsertTheme(Create("TextButton", TopRow, {
        Size = UDim2.new(0, btnWidth, 0, 22),
        Position = UDim2.new(1, 0, 0.5),
        AnchorPoint = Vector2.new(1, 0.5),
        Text = hasBtnText and CButtonText or "",
        Font = Enum.Font.GothamBold,
        TextSize = 9,
      ...
      Make("Corner", ActionButton, UDim.new(0, 4))

      local ButtonIcon = nil
      if hasBtnIcon then
        ButtonIcon = Create("ImageLabel", ActionButton, {
          Size = hasBtnText and UDim2.new(0, 10, 0, 10) or UDim2.new(0, 14, 0, 14),
          Position = hasBtnText and UDim2.new(0, 3, 0.5) or UDim2.new(0.5, 0, 0.5),
          AnchorPoint = hasBtnText and Vector2.new(0, 0.5) or Vector2.new(0.5, 0.5),
          Image = CButtonIcon,
          BackgroundTransparency = 1,
          ImageColor3 = Color3.fromRGB(245, 245, 245)
        })
        if hasBtnText then
          ActionButton.TextXAlignment = Enum.TextXAlignment.Right
          Create("UIPadding", ActionButton, { PaddingRight = UDim.new(0, 4) })
        end
      end

      -- Button hover
      ActionButton.MouseEnter:Connect(function()
        CreateTween({ActionButton, "BackgroundTransparency", 0.2, 0.2})
      end)
      ActionButton.MouseLeave:Connect(function()
        CreateTween({ActionButton, "BackgroundTransparency", 0, 0.2})
      end)
      ...
      local EventCard = {}
      function EventCard:Visible(...) Funcs:ToggleVisible(CardHolder, ...) end
      function EventCard:Destroy() CardHolder:Destroy() end

      function EventCard:SetTitle(New)
        TitleLabel.Text = tostring(New)
      end
      function EventCard:SetButtonText(New)
        ActionButton.Text = tostring(New)
        if ButtonIcon then
          if New and New:gsub(" ", ""):len() > 0 then
            ActionButton.TextXAlignment = Enum.TextXAlignment.Right
            ButtonIcon.Position = UDim2.new(0, 3, 0.5)
            ButtonIcon.AnchorPoint = Vector2.new(0, 0.5)
          else
            ActionButton.TextXAlignment = Enum.TextXAlignment.Center
            ButtonIcon.Position = UDim2.new(0.5, 0, 0.5)
            ButtonIcon.AnchorPoint = Vector2.new(0.5, 0.5)
          end
        end
      end
      function EventCard:SetImage(New)
        if not New:find("rbxassetid://") then
          New = genesislib:GetIcon(New)
        end
        CardImage.Image = New
      end
      function EventCard:SetButtonIcon(New)
        if not New:find("rbxassetid://") then
          New = genesislib:GetIcon(New)
        end
        if ButtonIcon then
          ButtonIcon.Image = New
        else
          -- Criar ícone se não existia
          ButtonIcon = Create("ImageLabel", ActionButton, {
            Size = ActionButton.Text:gsub(" ", ""):len() > 0 and UDim2.new(0, 12, 0, 12) or UDim2.new(0, 14, 0, 14),
            Position = ActionButton.Text:gsub(" ", ""):len() > 0 and UDim2.new(0, 6, 0.5) or UDim2.new(0.5, 0, 0.5),
            AnchorPoint = ActionButton.Text:gsub(" ", ""):len() > 0 and Vector2.new(0, 0.5) or Vector2.new(0.5, 0.5),
            Image = New,
            BackgroundTransparency = 1,
            ImageColor3 = Color3.fromRGB(245, 245, 245)
          })
      ...
      function EventCard:SetSubtitles(NewSubs)
        -- Limpar antigos
        for _, lbl in ipairs(SubtitleLabels) do
          lbl:Destroy()
        end
        table.clear(SubtitleLabels)
        -- Criar novos
        for i, sub in ipairs(NewSubs) do
          local lbl = InsertTheme(Create("TextLabel", TextHolder, {
            Size = UDim2.new(1, 0, 0, 10),
            BackgroundTransparency = 1,
            Font = Enum.Font.Gotham,
      ...
      function EventCard:SetSubtitle(Index, NewText)
        local lbl = SubtitleLabels[Index]
        if lbl then
          lbl.Text = tostring(NewText)
        end
      end
      function EventCard:GetSubtitleLabel(Index)
        return SubtitleLabels[Index]
      end
      function EventCard:Callback(NewCb)
        Callback = NewCb
      end
      return EventCard
    end
  ...
  -- ========== Background / Glass / Blur API ==========

  local function ClearImageBlurLayers(self)
    if not self.BackgroundImage then return end
    local parent = self.BackgroundImage.Parent
    if not parent then return end
    for _, child in ipairs(parent:GetChildren()) do
      if child.Name == "BlurCopy" then
        pcall(function() child:Destroy() end)
      end
    end
  end

  local function ApplyGlassVisual(self)
    local level = math.clamp(tonumber(self._glassLevel) or 0, 0, 1)
    if self.GlassLayer then
      if level <= 0 then
        self.GlassLayer.BackgroundTransparency = 1
        if GlassStroke then GlassStroke.Transparency = 1 end
      else
        self.GlassLayer.BackgroundColor3 = Color3.fromRGB(240, 240, 255)
        self.GlassLayer.BackgroundTransparency = 1 - (level * 0.35)
        if GlassStroke then
          GlassStroke.Transparency = 1 - (level * 0.55)
        end
      end
    end
  end

  -- Blur SÓ na imagem de fundo (stack de cópias offset) — não afeta a tela do jogo
  local function ApplyBlurVisual(self)
    local level = math.clamp(tonumber(self._blurLevel) or 0, 0, 1)
    ClearImageBlurLayers(self)
    if level <= 0 or not self.BackgroundImage then return end
    if not self.BackgroundImage.Image or self.BackgroundImage.Image == "" then return end

    local parent = self.BackgroundImage.Parent
    if not parent then return end

    local copies = math.clamp(math.floor(level * 10) + 2, 2, 12)
    local spread = 1 + level * 5
    local baseZ = self.BackgroundImage.ZIndex or 1
    self.BackgroundImage.ZIndex = baseZ + copies + 1

    for i = 1, copies do
      local ang = (i / copies) * math.pi * 2
      local ox = math.cos(ang) * spread
      local oy = math.sin(ang) * spread
      local copy = self.BackgroundImage:Clone()
      copy.Name = "BlurCopy"
      copy.ZIndex = baseZ + i
      copy.AnchorPoint = Vector2.new(0.5, 0.5)
      copy.Position = UDim2.new(0.5, ox, 0.5, oy)
      copy.Size = UDim2.new(1, spread * 2, 1, spread * 2)
      -- cada cópia bem transparente = efeito de desfoque
      copy.ImageTransparency = math.clamp(0.55 + (level * 0.25), 0.5, 0.9)
      copy.BackgroundTransparency = 1
      copy.Parent = parent
    end
  end

  local function ApplyOverlayVisual(self)
    if not self.BackgroundOverlay then return end
    local t = tonumber(self._bgOverlayTransparency)
    if t == nil then t = 0.45 end
    self.BackgroundOverlay.BackgroundColor3 = Theme["Color Hub 2"]
    self.BackgroundOverlay.BackgroundTransparency = math.clamp(t, 0, 1)
  end

  function Window:SetGlassEffect(level)
    level = math.clamp(tonumber(level) or 0, 0, 1)
    self._glassLevel = level
    ApplyGlassVisual(self)
  end

  function Window:SetBlur(level)
    level = math.clamp(tonumber(level) or 0, 0, 1)
    self._blurLevel = level
    ApplyBlurVisual(self)
  end

  -- 0 = overlay opaco (bem escuro) | 1 = sem overlay
  function Window:SetBackgroundTransparency(transparency)
    self._bgOverlayTransparency = math.clamp(tonumber(transparency) or 0.45, 0, 1)
    ApplyOverlayVisual(self)
  end

  -- 0 = imagem nítida | 1 = imagem totalmente invisível
  function Window:SetBackgroundImageTransparency(transparency)
    transparency = math.clamp(tonumber(transparency) or 0, 0, 1)
    self._bgImageTransparency = transparency
    if self.BackgroundImage then
      self.BackgroundImage.ImageTransparency = transparency
    end
  end

  -- Darkness 0 = claro | 1 = bem escuro (inverso da transparency do overlay)
  function Window:SetBackgroundDarkness(darkness)
    darkness = math.clamp(tonumber(darkness) or 0.55, 0, 1)
    self._bgOverlayTransparency = 1 - darkness
    ApplyOverlayVisual(self)
  end

  function Window:SetBackground(Image, Configs)
    Configs = Configs or {}
    local OverlayTransparency = Configs.OverlayTransparency or Configs.Transparency
    local ImageTransparency = Configs.ImageTransparency or Configs.ImageAlpha
    local ScaleType = Configs.ScaleType or "Crop"
    local ImageColor3 = Configs.ImageColor3 or Color3.fromRGB(255, 255, 255)
    local AutoDownload = Configs.AutoDownload ~= false
    local CacheFolder = Configs.CacheFolder or "GenesisLib/Assets"
    local Darken = Configs.Darkness or Configs.Darken
    local Blur = Configs.Blur
    local Glass = Configs.GlassEffect or Configs.Glass

    if not self.BackgroundImage or not self.ThemeBg or not self.BackgroundOverlay then
      warn("[genesislib] SetBackground: background elements missing")
      return
    end

    local function ResolveImage(img)
      return genesislib:ResolveImage(img, {
    ...
    if not resolved then
      ...
      self.ThemeBg.BackgroundTransparency = 0.03
      self.BackgroundOverlay.BackgroundTransparency = 1
      return
    end

    self.BackgroundImage.Image = resolved
    self.BackgroundImage.ScaleType = Enum.ScaleType[ScaleType] or Enum.ScaleType.Crop
    self.BackgroundImage.ImageColor3 = ImageColor3
    self.ThemeBg.BackgroundTransparency = 1
    -- re-aplica blur na imagem se já estiver ativo
    if (self._blurLevel or 0) > 0 then
      ApplyBlurVisual(self)
    end

    -- Transparency / Darkness do overlay
    if Darken ~= nil then
      self._bgOverlayTransparency = 1 - math.clamp(tonumber(Darken) or 0.55, 0, 1)
    elseif OverlayTransparency ~= nil then
      self._bgOverlayTransparency = math.clamp(tonumber(OverlayTransparency) or 0.45, 0, 1)
    else
      -- default um pouco mais escuro
      if self._bgOverlayTransparency == nil then
        self._bgOverlayTransparency = 0.45
      end
    end
    ApplyOverlayVisual(self)

    if ImageTransparency ~= nil then
      self:SetBackgroundImageTransparency(ImageTransparency)
    else
      self.BackgroundImage.ImageTransparency = self._bgImageTransparency or 0
    end
    ...
    if Glass ~= nil then
      self:SetGlassEffect(Glass)
    end

    if not self._BGThemeConnected then
      self._BGThemeConnected = true
      if Connection and Connection.ThemeChanged then
  ...
  function Window:RemoveBackground()
    if not self.BackgroundImage then return end
    self.BackgroundImage.Image = ""
    self.BackgroundImage.ImageTransparency = 0
  ...
  -- aplica glass/blur iniciais do MakeWindow
  if WGlass > 0 then
    Window:SetGlassEffect(WGlass)
  end
  if WBlur > 0 then
    Window:SetBlur(WBlur)
  end

  MainFrame.Destroying:Connect(function()
    pcall(function() ClearImageBlurLayers(Window) end)
  end)
...
-- Notificações: ZIndex alto pra ficar ACIMA do Hub inteiro
...
local NotifyLayout = Create("UIListLayout", NotificationHolder, {
...
function genesislib:Notify(Configs)
  local NTitle = Configs[1] or Configs.Title or "Notification"
  local NText = Configs[2] or Configs.Text or ""
  local NIcon = Configs[3] or Configs.Icon or nil
  local NDuration = Configs[4] or Configs.Duration or 5
  local NButtons = Configs[5] or Configs.Buttons or nil

  -- Resolve icon (assetid, URL, nome lucide)
  if NIcon then
    NIcon = genesislib:ResolveImage(tostring(NIcon))
  end
  local hasIcon = NIcon and type(NIcon) == "string" and NIcon:len() > 0 and NIcon ~= "nil"

  -- Criar notificação
  local NotifFrame = InsertTheme(Create("Frame", NotificationHolder, {
  ...
  Make("Corner", NotifFrame, UDim.new(0, 6))
  Make("Gradient", NotifFrame, { Rotation = 60 })

  local NotifPadding = Create("UIPadding", NotifFrame, {
    PaddingLeft = UDim.new(0, 10),
    PaddingRight = UDim.new(0, 10),
  ...
  local NotifList = Create("UIListLayout", NotifFrame, {
    SortOrder = "LayoutOrder",
    Padding = UDim.new(0, 6)
  })

  -- Top row: Icon + Text
  local TopRow = Create("Frame", NotifFrame, {
    Size = UDim2.new(1, 0, 0, 0),
    AutomaticSize = "Y",
    BackgroundTransparency = 1,
    LayoutOrder = 1
  ...
  local IconLabel = nil
  if hasIcon then
    IconLabel = Create("ImageLabel", TopRow, {
    ...
    Make("Corner", IconLabel, UDim.new(0, 5))
  end

  local TextHolder = Create("Frame", TopRow, {
    Size = UDim2.new(1, hasIcon and -30 or 0, 0, 0),
    Position = UDim2.new(0, hasIcon and 28 or 0, 0, 0),
    AutomaticSize = "Y",
    BackgroundTransparency = 1
  })
  ...
  local TitleLbl = InsertTheme(Create("TextLabel", TextHolder, {
  ...
  local TextLbl = nil
  if NText and NText:gsub(" ", ""):len() > 0 then
    TextLbl = InsertTheme(Create("TextLabel", TextHolder, {
      Size = UDim2.new(1, 0, 0, 0),
      AutomaticSize = "Y",
      BackgroundTransparency = 1,
      Font = Enum.Font.Gotham,
  ...
  -- Progress bar (se tiver duração > 0)
  local ProgressBar = nil
  if NDuration > 0 then
    local BarHolder = Create("Frame", NotifFrame, {
      Size = UDim2.new(1, 0, 0, 2),
      BackgroundTransparency = 1,
      LayoutOrder = 2
    })
    ProgressBar = InsertTheme(Create("Frame", BarHolder, {
      Size = UDim2.new(1, 0, 1, 0),
      BackgroundColor3 = Theme["Color Theme"],
      BorderSizePixel = 0
    }), "Theme")
  ...
  -- Buttons (se houver)
  local ButtonsHolder = nil
  if NButtons and type(NButtons) == "table" and #NButtons > 0 then
    ButtonsHolder = Create("Frame", NotifFrame, {
      Size = UDim2.new(1, 0, 0, 0),
      AutomaticSize = "Y",
      BackgroundTransparency = 1,
      LayoutOrder = 3
    })
    local BtnList = Create("UIListLayout", ButtonsHolder, {
      Padding = UDim.new(0, 6),
      FillDirection = "Horizontal",
      HorizontalAlignment = "Right"
    })

    for _, btnConfig in ipairs(NButtons) do
      local bName = btnConfig[1] or btnConfig.Name or "OK"
      local bCallback = btnConfig[2] or btnConfig.Callback or function() end
      ...
      Create("UIPadding", Btn, {
        PaddingLeft = UDim.new(0, 10),
        PaddingRight = UDim.new(0, 10)
      })

      Btn.MouseEnter:Connect(function()
        CreateTween({Btn, "BackgroundTransparency", 0.25, 0.15})
      end)
  ...
  local CloseBtn = Create("ImageButton", NotifFrame, {
    Size = UDim2.new(0, 14, 0, 14),
  ...
  -- Animação de entrada
  NotifFrame.Position = UDim2.new(0, 30, 0, 0)
  CreateTween({NotifFrame, "Position", UDim2.new(0, 0, 0, 0), 0.35})
  CreateTween({NotifFrame, "BackgroundTransparency", 0, 0.25})
  ...
  local function CloseNotif()
    ...
    CreateTween({NotifFrame, "Position", UDim2.new(0, 30, 0, 0), 0.3})
    CreateTween({NotifFrame, "BackgroundTransparency", 1, 0.25})
  ...
  if NDuration > 0 then
    Connection = RunService.Heartbeat:Connect(function(dt)
      ...
      Elapsed = Elapsed + dt
      if ProgressBar then
        ProgressBar.Size = UDim2.new(1 - (Elapsed / NDuration), 0, 1, 0)
      end
      if Elapsed >= NDuration then
        CloseNotif()
      end
    end)
  end
  ...
  -- Retornar controle
  local NotifyObj = {}
  function NotifyObj:Close() CloseNotif() end
  function NotifyObj:SetTitle(New) TitleLbl.Text = tostring(New) end
  function NotifyObj:SetText(New)
    if TextLbl then
      TextLbl.Text = tostring(New)
    end
  end
  return NotifyObj
end
